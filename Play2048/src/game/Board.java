...
public class Board {
    ...
    public void updateOpenSpaces() {
        ...
        openSpaces = new ArrayList<>(); // <item type>(capacity) // (1)
        
        for (int i = 0; i < gameBoard.length; i++){
            for (int j = 0; j < gameBoard[i].length; j++){
                    if (gameBoard[i][j] == 0){
                        BoardSpot pair = new BoardSpot(i,j);
                        openSpaces.add(pair); // (2)
                }
            }
        }
    }
    ...
    public void addRandomTile() {
        // WRITE YOUR CODE HERE
        int size = openSpaces.size(); // size of arraylist
        int random = StdRandom.uniform(0,size); // random index integer
    
        int i = openSpaces.get(random).getRow(); // pick random BoardSpot
        int j = openSpaces.get(random).getCol();

        // update value of tile
        ...
        // get rid of open space
        openSpaces.remove(random);
    }
    ...
    public void swipeLeft() {
        // WRITE YOUR CODE HERE
        int empty = 0;
        for (int r = 0; r < gameBoard.length; r++){ //iterate for each row
            for (int c = 0; c < gameBoard[r].length; c++){ //iterate for each column
                if (gameBoard[r][c] == 0){ // found empty space
                    empty = c;
                    for (int k = c+1; k < gameBoard[r].length; k++){ //iterate rest of col
                        if (gameBoard[r][k] != 0){ // found nonempty
                            gameBoard[r][empty] = gameBoard[r][k]; // put nonempty into empty space
                            gameBoard[r][k] = 0; // make spot empty
                            empty = k; // update empty spot
                        }
                    }
                }
            }
    ...
    public void mergeLeft() {
        // WRITE YOUR CODE HERE
        for (int r = 0; r < gameBoard.length; r++){
            for (int c = 0; c < gameBoard[r].length-1; c++){ // -1 because c+1 in next line
                if (gameBoard[r][c] == gameBoard[r][c+1]){
                    int sum = 2 * gameBoard[r][c];
                    gameBoard[r][c] = sum;
                    gameBoard[r][c+1] = 0;
                }
            }
    ...
    public void flipRows() {
        ...
        for (int r = 0; r < gameBoard.length; r++){
            for (int c = 0; c < gameBoard[r].length; c++){
                gameBoard[r][c] = copy[r][3-count]; // index 3 because only 4 col
                count++;
            }
            count = 0;
        }
